...
function render(a, likes) {
  ...
  var parts = [
    ...
    renderScript(a.id, md),
    '</body></html>'
  ];
  return parts.join('\n');
}

function renderScript(aid, mdEscaped) {
  return '<script>var aid=' + aid + ',fp=localStorage.getItem("fp")||(function(){var f="fp"+Date.now()+Math.random();localStorage.setItem("fp",f);return f;})(),replyTo=null;' +
    'function init(){if(typeof marked==="undefined"){setTimeout(init,100);return;}document.getElementById("content").innerHTML=marked.parse(' + mdEscaped + ');enhanceCodeBlocks();}init();loadComments();updateLikeState();' +
    'function enhanceCodeBlocks(){var pres=document.querySelectorAll(".article-body pre");for(var i=0;i<pres.length;i++){(function(pre){' +
    'var code=pre.querySelector("code"),lang="";if(code){var m=(code.className||"").match(/language-(\\w+)/);if(m)lang=m[1];}' +
    'var wrapper=document.createElement("div");wrapper.className="code-block-wrapper";' +
    'var bar=document.createElement("div");bar.className="code-block-bar";bar.innerHTML=' + JSON.stringify('<span class="code-block-lang"></span><button class="code-block-btn" data-action="expand" title="展开"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></button><span style="flex:1"></span><button class="code-block-btn" data-action="copy" title="复制"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg></button><button class="code-block-btn" data-action="fullscreen" title="全屏"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg></button>') + ';' +
    'bar.querySelector(".code-block-lang").textContent=lang||"code";' +
    'pre.parentNode.insertBefore(wrapper,pre);wrapper.appendChild(bar);wrapper.appendChild(pre);' +
    'bar.addEventListener("click",function(e){var btn=e.target.closest(".code-block-btn");if(!btn)return;' +
    'var action=btn.dataset.action,wrapper=btn.closest(".code-block-wrapper"),preEl=wrapper.querySelector("pre");' +
    'if(action==="copy"){var text=preEl.textContent;navigator.clipboard.writeText(text).then(function(){' +
    'var orig=btn.innerHTML;btn.innerHTML=' + JSON.stringify('<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>') + ';' +
    'setTimeout(function(){btn.innerHTML=orig},1500)})}' +
    'else if(action==="expand"){preEl.classList.toggle("expanded");' +
    'var poly=btn.querySelector("polyline");if(poly)poly.setAttribute("points",preEl.classList.contains("expanded")?"18 15 12 9 6 15":"6 9 12 15 18 9")}' +
    'else if(action==="fullscreen"){wrapper.classList.toggle("fullscreen");' +
    'if(wrapper.classList.contains("fullscreen")){document.body.style.overflow="hidden";' +
    'if(!preEl.querySelector(".line-numbers")){var lines=preEl.textContent.split("\\n"),nums="",lc=0;' +
    ...
    'preEl.insertBefore(ln,preEl.firstChild);preEl.style.display="flex";preEl.style.alignItems="flex-start"}' +
    '}else{document.body.style.overflow=""}function onEsc(e){if(e.key==="Escape"){wrapper.classList.remove("fullscreen");document.body.style.overflow="";document.removeEventListener("keydown",onEsc)}}document.addEventListener("keydown",onEsc)}})' +
    '})(pres[i])}}' +
