...
function render(a, likes) {
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + esc(a.title) + '</title><link rel="stylesheet" href="/style.css"><\/head><body>' +
    ...
    'function onCBBtn(e){var btn=e.target.closest(".code-block-btn");var w=e.target.closest(".code-block-wrapper");if(!w)return;if(btn){var a=btn.dataset.a,body=w.querySelector(".cb-body"),pre=w.querySelector("pre");if(a==="copy")navigator.clipboard.writeText(pre.textContent);if(a==="fullscreen"){w.classList.toggle("fullscreen");if(w.classList.contains("fullscreen")){document.body.style.overflow="hidden";if(!pre.querySelector(".line-numbers")){var lines=pre.textContent.split("\\n"),nums="",lc=0;for(var li=0;li<lines.length;li++){if(lines[li].trim()||li<lines.length-1)nums+=(++lc)+"\\n"}var ln=document.createElement("div");ln.className="line-numbers";ln.textContent=nums;pre.insertBefore(ln,pre.firstChild);pre.style.display="flex"}}else{document.body.style.overflow=""}}}else{var body=w.querySelector(".cb-body");body.classList.toggle("hidden");w.classList.toggle("folded")}}' +
