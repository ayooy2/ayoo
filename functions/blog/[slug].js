...
function render(a, likes) {
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + esc(a.title) + '</title><link rel="stylesheet" href="/style.css"><\/head><body>' +
    ...
    'function barHTML(l){return' + JSON.stringify('<span class="code-block-lang">X</span><button class="code-block-btn" data-a="expand"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg><span>收起</span></button><span style="flex:1"></span><button class="code-block-btn" data-a="copy"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg><span>复制</span></button><button class="code-block-btn" data-a="fullscreen"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg><span>全屏</span></button>') + '.replace("X",l)}' +
    'function wrapCB(){var pres=document.querySelectorAll("#content pre:not(.code-block-wrapper pre)");for(var i=0;i<pres.length;i++){var pre=pres[i],code=pre.querySelector("code"),lang="";if(code){var m=(code.className||"").match(/language-(\\w+)/);if(m)lang=m[1];}var w=document.createElement("div");w.className="code-block-wrapper";var bar=document.createElement("div");bar.className="code-block-bar";bar.innerHTML=barHTML(lang||"text");var body=document.createElement("div");body.className="cb-body";pre.parentNode.insertBefore(w,pre);w.appendChild(bar);body.appendChild(pre);w.appendChild(body)}}' +
    'document.addEventListener("DOMContentLoaded",function(){document.getElementById("content").addEventListener("click",onCBBtn)});' +
    'function onCBBtn(e){var btn=e.target.closest(".code-block-btn");if(!btn)return;var a=btn.dataset.a,w=btn.closest(".code-block-wrapper"),body=w.querySelector(".cb-body"),pre=w.querySelector("pre");switch(a){case"copy":navigator.clipboard.writeText(pre.textContent);break;case"expand":body.classList.toggle("hidden");var sp=btn.querySelector("span");if(sp)sp.textContent=body.classList.contains("hidden")?"展开":"收起";break;case"fullscreen":w.classList.toggle("fullscreen");if(w.classList.contains("fullscreen")){document.body.style.overflow="hidden";if(!pre.querySelector(".line-numbers")){var lines=pre.textContent.split("\\n"),nums="",lc=0;for(var li=0;li<lines.length;li++){if(lines[li].trim()||li<lines.length-1)nums+=(++lc)+"\\n"}var ln=document.createElement("div");ln.className="line-numbers";ln.textContent=nums;pre.insertBefore(ln,pre.firstChild);pre.style.display="flex"}}else{document.body.style.overflow=""}break}}' +
...
function simpleMD(md) {
  ...
  for (var n = 0; n < cbs.length; n++) {
    ...
    var html = '<div class="code-block-wrapper"><div class="code-block-bar">' +
      ...
      '<button class="code-block-btn" data-a="expand"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg><span>收起</span></button>' +
      '<span style="flex:1"></span>' +
      '<button class="code-block-btn" data-a="copy"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg><span>复制</span></button>' +
      '<button class="code-block-btn" data-a="fullscreen"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg><span>全屏</span></button>' +
