...
function render(a, likes) {
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + esc(a.title) + '</title>' + seo + '<link rel="stylesheet" href="/style.css"><\/head><body>' +
    ...
    'function postComment(btn){var c=document.getElementById("comment-input").value.trim(),n=document.getElementById("comment-name").value.trim()||"匿名";if(!c){if(btn)btn.disabled=false;return;}fetch("/api/comments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({article_id:aid,parent_id:replyTo,author_name:n,content:c})}).then(function(r){return r.json()}).then(function(){document.getElementById("comment-input").value="";replyTo=null;loadComments()}).finally(function(){if(btn)btn.disabled=false})}' +
