...
function rc(list,d){
  ...
  for(var i=0;i<list.length;i++){
    ...
    if(avatar.indexOf("d=404")>0&&_cEmail&&c.author_name===_cName){avatar=gravatar(_cEmail);}
    var avatarHash=(avatar.match(/avatar\\/([a-f0-9]{32})/)||["",""])[1];
    var fallback=avatarHash?identiconSvg(avatarHash):"https://cravatar.cn/avatar?d=mp&amp;s=48";
    var nameHtml=c.url
    ...
    h+='<img class="comment-avatar" src="'+(avatar||fallback)+'" alt="" loading="lazy" onerror="this.src=&quot;'+fallback.replace(/&/g,'&amp;')+'&quot;">';
...
function gravatar(email){if(!email)return"";return"https://cravatar.cn/avatar/"+md5c(email.trim().toLowerCase())+"?d=404&s=48";}
