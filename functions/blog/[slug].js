...
function render(a, likes) {
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + esc(a.title) + '</title><link rel="stylesheet" href="/style.css"><\/head><body>' +
    ...
    'function onCBBtn(e){var btn=e.target.closest(".code-block-btn");if(!btn)return;var a=btn.dataset.a,w=btn.closest(".code-block-wrapper"),pre=w.querySelector("pre");switch(a){case"copy":navigator.clipboard.writeText(pre.textContent);break;case"expand":pre.classList.toggle("expanded");var pl=btn.querySelector("polyline");if(pl)pl.setAttribute("points",pre.classList.contains("expanded")?"18 15 12 9 6 15":"6 9 12 15 18 9");break;case"fullscreen":w.classList.toggle("fullscreen");if(w.classList.contains("fullscreen")){document.body.style.overflow="hidden";if(!pre.querySelector(".line-numbers")){var lines=pre.textContent.split("\\n"),nums="",lc=0;for(var li=0;li<lines.length;li++){if(lines[li].trim()||li<lines.length-1)nums+=(++lc)+"\\n"}var ln=document.createElement("div");ln.className="line-numbers";ln.textContent=nums;pre.insertBefore(ln,pre.firstChild);pre.style.display="flex"}}else{document.body.style.overflow=""}break}}' +
