...
function render(a, likes) {
  ...
  h += 'function init(){if(typeof marked==="undefined"){setTimeout(init,100);return;}document.getElementById("content").innerHTML=marked.parse(' + md + ');enhanceCodeBlocks();}init();loadComments();updateLikeState();';
  h += 'function enhanceCodeBlocks(){var pres=document.querySelectorAll(".article-body pre");for(var i=0;i<pres.length;i++){(function(pre){var code=pre.querySelector("code");var lang="";var cls=code?code.className||"":"";var m=cls.match(/language-(\w+)/);if(m)lang=m[1];var wrapper=document.createElement("div");wrapper.className="code-block-wrapper";var bar=document.createElement("div");bar.className="code-block-bar";bar.innerHTML=\'<span class="code-block-lang">\'+(lang||"code")+\'</span><button class="code-block-btn" title="复制" onclick="copyCode(this)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg></button><button class="code-block-btn" title="展开" onclick="toggleExpand(this)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></button><button class="code-block-btn" title="全屏" onclick="toggleFullscreen(this)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg></button>\';pre.parentNode.insertBefore(wrapper,pre);wrapper.appendChild(bar);wrapper.appendChild(pre)})(pres[i])}}';
  h += 'function copyCode(btn){var pre=btn.parentElement.parentElement.querySelector("pre code");var text=pre?pre.textContent:"";navigator.clipboard.writeText(text).then(function(){var orig=btn.innerHTML;btn.innerHTML=\'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>\';setTimeout(function(){btn.innerHTML=orig},1500)})}';
  h += 'function toggleExpand(btn){var pre=btn.parentElement.parentElement.querySelector("pre");pre.classList.toggle("expanded");var svg=btn.querySelector("svg");if(pre.classList.contains("expanded")){svg.innerHTML=\'<polyline points="18 15 12 9 6 15"/>\'}else{svg.innerHTML=\'<polyline points="6 9 12 15 18 9"/>\'}}';
  h += 'function toggleFullscreen(btn){var w=btn.parentElement.parentElement;w.classList.toggle("fullscreen");if(w.classList.contains("fullscreen")){document.body.style.overflow="hidden";w.querySelector(".code-block-btn[title=\\"全屏\\"] svg").innerHTML=\'<polyline points="4 14 10 14 10 20"/><polyline points="20 10 14 10 14 4"/><line x1="14" y1="10" x2="21" y2="3"/><line x1="3" y1="21" x2="10" y2="14"/>\'}else{document.body.style.overflow="";w.querySelector(".code-block-btn[title=\\"全屏\\"] svg").innerHTML=\'<polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/>\'}document.addEventListener("keydown",function esc(e){if(e.key==="Escape"){w.classList.remove("fullscreen");document.body.style.overflow="";document.removeEventListener("keydown",esc)}})}';
