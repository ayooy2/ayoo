...
export async function requireAuth(request, env, password) {
  ...
  try {
    ...
    if (env.ADMIN_PASSWORD && token === env.ADMIN_PASSWORD) {
      ...
      const newHash = await hashPassword(token);
      await env.DB.prepare(
        "INSERT INTO settings (key, value, updated_at) VALUES ('admin_password_hash', ?, CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=?, updated_at=CURRENT_TIMESTAMP"
